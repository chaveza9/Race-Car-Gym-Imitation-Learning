...
def load_imitations(data_folder):
    ...
    """
    # Extract file locations from folder path
    action_files = glob.glob( data_folder+"/action_*.npy")
    observation_files = glob.glob( data_folder+"/observation_*.npy")
    # Create empty lists
    observations = []
    actions = []
    # Iterate through each file
    for action in action_files:
        actions.append(np.load(action))
    for observation in observation_files:
        observations.append(np.load(observation))
    # Return lists
